...
const MINIMUM_FEEDBACK_FOR_SHELF = 1; // Added missing constant
...
function getPostCreditStatus(userId, member) {
    ...
    if (user.currentCredits >= 1) {
        return `✅ ${user.currentCredits} credit${user.currentCredits === 1 ? '' : 's'} available`;
    } else {
        return `📝 Need more feedback to post another chapter`;
    }
}
...
async function resetUserProgress(userId, guild) {
    ...
    await closeUserBookshelfThreads(guild, userId);
    
    await saveData();
    console.log(`User ${userId} progress completely reset`);
}
...
client.on('guildMemberAdd', async (member) => {
    ...
    const welcomeChannel = member.guild.channels.cache.find(ch => 
        ch.name.includes('welcome') || ch.name === 'general' || ch.type === 0
    );
    ...
    if (welcomeChannel) {
        const welcomeEmbed = new EmbedBuilder()
            ...
            .addFields(
                { name: '📜 Essential Reading', value: '• Review our server rules to understand our community standards\n• Peruse the server guide to learn how our realm operates', inline: false },
                { name: '🎭 Getting Started', value: '• Introduce yourself in the introductions channel with your lucky number and favorite animal\n• Select your roles in reaction-roles to personalize your experience', inline: false },
        ...
        try {
            ...
            console.log(`Welcome message sent for ${member.displayName}`);
        } catch (error) {
...
// ===== REMAINING COMMAND IMPLEMENTATIONS (keeping the same pattern as before) =====
