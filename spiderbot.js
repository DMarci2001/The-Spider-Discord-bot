...
client.on('guildMemberAdd', async (member) => {
    ...
    // Welcome channel announcement - specifically look for #welcome
    ...
    if (welcomeChannel) {
        const welcomeEmbed = new EmbedBuilder()
            ...
            .addFields(
                { name: '📜 Essential Reading', value: '• Review our **#rules** channel to understand our community standards\n• Peruse the **#server-guide** to learn how our realm operates', inline: false },
        ...
        try {
            ...
            console.log(`Welcome message sent for ${member.displayName} in #welcome`);
        } catch (error) {
...
function createCreditModificationEmbed(user, amount, result, action) {
    return new EmbedBuilder()
        ...
        .addFields(
            { name: 'Previous Balance', value: `${result.previousCredits}`, inline: true },
            { name: 'Current Balance', value: `${result.newCredits}`, inline: true },
            { name: `Credits ${action === 'removed' ? 'Removed' : 'Added'}`, value: `${amount}`, inline: true }
        )
