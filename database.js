...
class DatabaseManager {
    ...
    async createTables() {
        try {
            // Create all tables with individual statements for better control
            await this.db.run(`
            ...
            // Create pardoned_users table with new schema (ONLY ONCE)
            await this.db.run(`
                CREATE TABLE IF NOT EXISTS pardoned_users (
                    user_id TEXT NOT NULL,
                    month_key TEXT NOT NULL,
                    reason TEXT DEFAULT 'staff_discretion',
                    pardoned_at INTEGER DEFAULT (strftime('%s', 'now') * 1000),
                    PRIMARY KEY (user_id, month_key)
                )
            `);

            // ===== MIGRATION FOR EXISTING PARDONED_USERS TABLE =====
            // Check if the reason column exists, if not, add it
            const tableInfo = await this.db.all('PRAGMA table_info(pardoned_users)');
            const hasReasonColumn = tableInfo.some(column => column.name === 'reason');

            if (!hasReasonColumn) {
                console.log('🔄 Migrating pardoned_users table to include reason column...');
                await this.db.run('ALTER TABLE pardoned_users ADD COLUMN reason TEXT DEFAULT "staff_discretion"');
                console.log('✅ Added reason column to pardoned_users table');
            }
            ...
            await this.db.run(`
                CREATE TABLE IF NOT EXISTS shame_cooldowns (
                    user_id TEXT PRIMARY KEY,
    ...
    // ===== UPDATED PARDON METHODS FOR ENHANCED SYSTEM =====
    async pardonUser(userId, monthKey, reason = 'staff_discretion') {
        try {
            await this.db.run('INSERT OR REPLACE INTO pardoned_users (user_id, month_key, reason) VALUES (?, ?, ?)', [userId, monthKey, reason]);
        } catch (error) {
    ...
    async isUserPardoned(userId, monthKey) {
        try {
            const result = await this.db.get('SELECT reason FROM pardoned_users WHERE user_id = ? AND month_key = ?', [userId, monthKey]);
            return !!result;
        } catch (error) {
    ...
    async getPardonedUsersForMonth(monthKey) {
        try {
            return await this.db.all('SELECT user_id, reason FROM pardoned_users WHERE month_key = ?', [monthKey]);
        } catch (error) {
    ...
    async deleteUser(userId) {
        try {
            ...
            await this.db.run('DELETE FROM faceless_cooldowns WHERE user_id = ?', [userId]);
            await this.db.run('DELETE FROM shame_cooldowns WHERE user_id = ?', [userId]);
        } catch (error) {
    ...
    async cleanupOldCooldowns() {
        try {
            ...
            await this.db.run('DELETE FROM shame_cooldowns WHERE last_used < ?', [cutoff]);
        } catch (error) {
